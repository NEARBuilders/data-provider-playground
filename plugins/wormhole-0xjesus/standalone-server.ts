/**
 * Standalone HTTP Server for Wormhole Plugin
...
async function initializeService() {
  console.log('[Standalone Server] Initializing Wormhole service...');

  const baseUrl = process.env.BASE_URL || 'https://api.wormholescan.io/api/v1';
  const apiKey = process.env.API_KEY || 'not-required';
  const timeout = parseInt(process.env.TIMEOUT || '15000');

  try {
    service = new DataProviderService(baseUrl, apiKey, timeout);
...
initializeService().then((success) => {
  if (success) {
    server.listen(PORT, () => {
      console.log(`\n${'='.repeat(60)}`);
      console.log(`🚀 Wormhole Plugin HTTP Server`);
