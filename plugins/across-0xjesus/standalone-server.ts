/**
 * Standalone HTTP Server for Across Plugin
...
async function initializeService() {
  console.log('[Standalone Server] Initializing Across service...');

  const baseUrl = process.env.BASE_URL || 'https://app.across.to/api';
  const coingeckoBaseUrl = process.env.COINGECKO_BASE_URL || 'https://api.coingecko.com/api/v3';
  const defillamaBaseUrl = process.env.DEFILLAMA_BASE_URL || 'https://bridges.llama.fi';
  const apiKey = process.env.API_KEY || '';
  const timeout = parseInt(process.env.TIMEOUT || '30000');
  const maxRequestsPerSecond = parseInt(process.env.MAX_REQUESTS_PER_SECOND || '5');
...
initializeService().then((success) => {
  if (success) {
    server.listen(PORT, () => {
      console.log(`\n${'='.repeat(60)}`);
      console.log(`🚀 Across Plugin HTTP Server`);
