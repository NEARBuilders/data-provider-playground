/**
 * Standalone HTTP Server for deBridge Plugin
...
async function initializeService() {
  console.log('[Standalone Server] Initializing deBridge service...');

  const baseUrl = process.env.BASE_URL || 'https://dln.debridge.finance/v1.0';
...
initializeService().then((success) => {
  if (success) {
    server.listen(PORT, () => {
      console.log(`\n${'='.repeat(60)}`);
      console.log(`🚀 deBridge Plugin HTTP Server`);
