...
const TEST_REGISTRY: PluginRegistry = {
  "@MrSufferer/cctp": {
    ...
    version: "1.0.0",
    description: "Circle CCTP data provider plugin for integration testing",
  },
};
...
describe("CCTP Plugin Integration Tests", () => {
  ...
  beforeAll(async () => {
    const { initialized } = await runtime.usePlugin("@MrSufferer/cctp", TEST_CONFIG);
    expect(initialized).toBeDefined();
    expect(initialized.plugin.id).toBe("@MrSufferer/cctp");
  });
