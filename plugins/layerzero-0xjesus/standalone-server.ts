/**
 * Standalone HTTP Server for LayerZero Plugin
...
async function initializeService() {
  console.log('[Standalone Server] Initializing LayerZero service...');

  const baseUrl = process.env.BASE_URL || 'https://stargate.finance/api/v1';
...
initializeService().then((success) => {
  if (success) {
    server.listen(PORT, () => {
      console.log(`\n${'='.repeat(60)}`);
      console.log(`🚀 LayerZero Plugin HTTP Server`);
