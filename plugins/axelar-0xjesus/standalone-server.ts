/**
 * Standalone HTTP Server for Axelar Plugin
...
async function initializeService() {
  console.log('[Standalone Server] Initializing Axelar service...');

  const baseUrl = process.env.BASE_URL || 'https://dln.axelar.finance/v1.0';
...
initializeService().then((success) => {
  if (success) {
    server.listen(PORT, () => {
      console.log(`\n${'='.repeat(60)}`);
      console.log(`🚀 Axelar Plugin HTTP Server`);
