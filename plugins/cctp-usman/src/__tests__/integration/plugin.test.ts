...
const TEST_REGISTRY: PluginRegistry = {
  "@nearbuilders/cctp": {
    ...
    version: "1.0.0",
    description: "Circle CCTP data provider for NEAR Intents",
  },
};

const TEST_PLUGIN_MAP = {
  "@nearbuilders/cctp": CCTPPlugin,
} as const;
...
describe("CCTP Plugin Integration Tests", () => {
  ...
  beforeAll(async () => {
    const { initialized } = await runtime.usePlugin("@nearbuilders/cctp", TEST_CONFIG);
    expect(initialized).toBeDefined();
    expect(initialized.plugin.id).toBe("@nearbuilders/cctp");
  });

  describe("getSnapshot procedure", () => {
    it("should fetch complete snapshot successfully", async () => {
      const { client } = await runtime.usePlugin("@nearbuilders/cctp", TEST_CONFIG);
