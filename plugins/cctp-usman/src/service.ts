...
/**
 * CCTP-supported EVM chains with native USDC
 * 
 * Note: This list includes the primary EVM chains. CCTP also supports additional chains
 * including Solana, Linea, Sonic, Unichain, World Chain, Codex, Sei, and others.
 * 
 * For complete list of supported chains, see:
 * https://developers.circle.com/stablecoins/cctp-supported-blockchains
 */
const CCTP_CHAINS = {
...
export class CCTPService {
  ...
  getSnapshot(params: {
    ...
    return Effect.tryPromise({
      try: async () => {
        ...
        console.log(`[CCTPService] Fetching snapshot for ${params.routes?.length || 0} routes`);
   ...
   * 
   * CCTP V2 protocol fees:
   * - Standard Transfer: 0 bps (free) - https://developers.circle.com/stablecoins/cctp-finality-and-fees#cctp-v2-fees
   * - Fast Transfer: 1-14 bps depending on source chain (most chains: 1 bps, Linea: 14 bps)
   * 
   * Note: Some chains (Avalanche, Polygon PoS, Sonic, etc.) don't support Fast Transfer
   * because their standard attestation times are already very fast.
   * 
   * Fee API endpoint: GET /v2/burn/USDC/fees - https://developers.circle.com/api-reference/cctp/all/get-burn-usdc-fees
   * 
   * Currently using 6 bps as average fallback when fee API is unavailable.
   * For production use, integrate the fee API to get real-time, chain-specific fees.
   */
