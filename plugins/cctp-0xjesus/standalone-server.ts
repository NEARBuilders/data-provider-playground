/**
 * Standalone HTTP Server for CCTP Plugin
...
async function initializeService() {
  console.log('[Standalone Server] Initializing CCTP service...');

  const baseUrl = process.env.BASE_URL || 'https://iris-api.circle.com';
...
initializeService().then((success) => {
  if (success) {
    server.listen(PORT, () => {
      console.log(`\n${'='.repeat(60)}`);
      console.log(`🚀 CCTP Plugin HTTP Server`);
