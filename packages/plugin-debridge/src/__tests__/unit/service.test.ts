...
describe("DataProviderService - Unit Tests", () => {
  ...
  describe("getSnapshot", () => {
    ...
    it("should calculate effective rate correctly with decimal precision", async () => {
      ...
      // With operating expense prepended (2000) + 0.4% fee, effective rate should be ~0.994
      // Input: 1002000 (1000000 + 2000), Output: ~996000 (1000000 * 0.996)
      expect(rate.effectiveRate).toBeCloseTo(0.994, 2);
      
      // Amount out should be less than amount in
      const amountIn = parseFloat(rate.amountIn);
      const amountOut = parseFloat(rate.amountOut);
      expect(amountOut).toBeLessThan(amountIn);
      // The ratio should be around 0.994
      expect(amountOut / amountIn).toBeCloseTo(0.994, 2);
    });
